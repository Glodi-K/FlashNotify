    ...
    def send_notification_sync(self, app_instance, user_id: int, title: str, body: str, emergency_type: str = "académique") -> str:
        ...
        with app_instance.app_context():
            try:
                ...
                logging.info(f"Envoi de notification à l'utilisateur {user.email}")
                ...
                try:
                    ...
                    logging.warning(f"Erreur lors de la sauvegarde des métriques: {metric_error}")
...
def init_queues():
    ...
    try:
        ...
        logging.info("File d'attente ThreadPool démarrée")
        
        # Pour la production, on utilise uniquement ThreadPool pour éviter les problèmes asyncio
        # La file d'attente asynchrone sera désactivée en production
        if not os.environ.get('DATABASE_URL'):  # Mode développement
            try:
                ...
                try:
                    ...
                    logging.info("File d'attente asynchrone démarrée")
                finally:
                    asyncio.set_event_loop(None)
                    loop.close()
            except Exception as e:
                logging.warning(f"Impossible de démarrer la file d'attente asynchrone: {e}")
        
        logging.info("Files d'attente initialisées")
    except Exception as e:
        logging.error(f"Erreur lors de l'initialisation des files d'attente: {e}")
...
def shutdown_queues():
    ...
    try:
        ...
        # Arrêt de la file d'attente asynchrone seulement en mode développement
        if not os.environ.get('DATABASE_URL'):  # Mode développement
            try:
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                ...
                logging.warning(f"Erreur lors de l'arrêt de la file d'attente asynchrone: {e}")
        
        logging.info("Files d'attente arrêtées")
    except Exception as e:
        logging.error(f"Erreur lors de l'arrêt des files d'attente: {e}")